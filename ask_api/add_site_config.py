...
def get_cosmos_client():
    ...
    if not endpoint or not database_name:
        ...
        print()
        print(
            "These are the same variables used by object_storage (shared DB pattern)."
        )
        print(
            "For partitioned DB, set different variables and update config.yaml accordingly."
        )
        sys.exit(1)
...
def add_site_config(domain: str, config_data: dict):
    ...
    try:
        ...
        print(
            f"✓ Site config added/updated successfully for domain: {normalized_domain}"
        )
        print(f"  - Document ID: {config_id}")
        print(f"  - Partition key: {normalized_domain}")
        print(
            f"  - Intent-elicitation pairs: {len(config_data.get('intent_elicitations', []))}"
        )
        return result
...
def main():
    ...
    parser.add_argument(
        "--domain", required=True, help="Domain name (e.g., yelp.com, www.yelp.com)"
    )
