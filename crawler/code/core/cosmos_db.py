...
class CosmosDB:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Initialized - endpoint: {self.cosmos_endpoint}, database: {self.database_name}, container: {self.container_name}"
        )
        ...
        try:
            ...
            for item in items:
                ...
                try:
                    ...
                    logger.error(
                        f"❌ Unexpected error uploading {item.get('@id', 'unknown')}: {e}"
                    )
                    error_count += 1

            logger.info(
                f"Batch add complete - Success: {success_count}, Duplicates: {duplicate_count}, Errors: {error_count}"
            )
    ...
    async def batch_delete(self, object_ids: List[str]):
        ...
        try:
            ...
            for object_id in object_ids:
                try:
                    ...
                    def execute_delete():
                        try:
                            self.container.delete_item(
                                item=id_hash, partition_key=object_id
                            )
                            return True
                        except CosmosResourceNotFoundError:
                            return False
            ...
            logger.debug(
                f"Batch delete complete - Success: {success_count}, Not found: {not_found_count}, Errors: {error_count}"
            )
