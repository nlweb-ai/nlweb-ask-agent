...
class AzureOpenAIEmbedding:
    ...
    def __init__(
        self, endpoint: str, api_key: str, deployment: str = "text-embedding-3-small"
    ):
        """
    ...
    async def get_batch_embeddings(
        self, texts: List[str], retry_count: int = 0, max_retries: int = 8
    ) -> List[List[float]]:
        """
        ...
        try:
            ...
            if (
                ...
                if retry_count < max_retries:
                    ...
                    print(
                        f"[Embedding] Rate limit hit. Retry {retry_count + 1}/{max_retries} after {wait_time}s. Error: {error_msg[:200]}"
                    )
                    await asyncio.sleep(wait_time)
                    return await self.get_batch_embeddings(
                        texts, retry_count + 1, max_retries
                    )
                else:
                    print(
                        f"[Embedding] Max retries ({max_retries}) reached for rate limit. Failing batch."
                    )
                    raise Exception(
            ...
            if "maximum context length" in error_msg or "token" in error_msg.lower():
                ...
                if len(texts) > 1:
                    print(
                        f"[Embedding] Batch too large ({len(texts)} items), splitting in half and retrying..."
                    )
                    mid = len(texts) // 2
                    ...
                    print(
                        f"[Embedding] Single item too large, returning zero embedding. Error: {error_msg}"
                    )
