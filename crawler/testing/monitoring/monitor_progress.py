...
API_BASE = (
    "http://localhost:5001/api"  # Changed to 5001 to avoid macOS AirPlay conflict
)
            ...
            if queue_status:
                ...
                if queue_status["jobs"]:
                    ...
                    for job in queue_status["jobs"][:5]:
                        ...
                        print(
                            f"    {status_emoji} [{job['status']:10s}] {job['type']:20s}"
                        )
                        if job.get("file_url"):
            ...
            if site_status:
                ...
                if site_status:
                    ...
                    for site in site_status[:5]:
                        ...
                        print(
                            f"       Files: {site['total_files']:3d} | IDs: {site['total_ids']:5d} | "
    ...
    if args.once:
        ...
        if site_status:
            ...
            for site in site_status:
                print(
                    f"  {site['site_url']}: {site['total_files']} files, {site['total_ids']} IDs"
                )
    else:
        # Live monitoring
